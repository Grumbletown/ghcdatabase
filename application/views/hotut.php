<link href="<?php echo base_url('assets/css/hotut.css') ?>" type="text/css" rel="stylesheet" />
<script>
    setNavElemActive(["navHOInfo"]);
</script>
<div class="container">
    <h3 id="tutHeader">Tutorials und Guides für Hackerz Online</h2>
        <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                <div class="guideBorder col-md-12">
                    <div>
                        <a href="https://docs.google.com/document/d/18h_Ik023Ax9eGUxSCzVszhTask1y5ayP2TweVFNMdHE/pub"><h5>Tech-Guide</h5></a>
                    </div>
                    <div class="guideDescription">
                        Erlerne die Grundlagen des Spiels. Wie funktioniert was? Worauf kommt es an? Welche Skillungen gibt es? 
                        Diese Fragen werden hier beantwortet.
                    </div>
                    <div class="guideImage">
                        <img src="<?php echo base_url('assets/images/icon.png') ?>" alt="Bild/Symbol" width="50%" >
                    </div>
                </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                <div class="guideBorder col-md-12">
                    <div>
                        <a href="https://forum.hackerz.online/viewtopic.php?f=10&t=334"><h5>Taktik-Guide</h5></a>
                    </div>
                    <div class="guideDescription">
                        Erfahre, ob du auch die richtige Einstellung zum Spiel hast. Das Hacken hört nicht bei der Technik auf. 
                        Hacke den Spieler. "The Awakening" führt dich auf dem Pfad der Erleuchtung.
                    </div>
                    <div class="guideImage">
                        <img src="<?php echo base_url('assets/images/Hackerz_logo.png') ?>" alt="Bild/Symbol" width="50%" >
                    </div>
                </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                <div class="guideBorder col-md-12">
                    <div>
                        <a href="https://forum.hackerz.online/viewtopic.php?f=12&t=78"><h5>Gilden</h5></a>
                    </div>
                    <div class="guideDescription">
                        Geh in keine Gilde, bevor du das hier gelesen hast.
                    </div>
                    <div class="guideImage">
                        <img src="<?php echo base_url('assets/images/Hackerz_logo.png') ?>" alt="Bild/Symbol" width="50%" >
                    </div>
                </div>
            </div>

            <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                <div class="guideBorder col-md-12">
                    <div>
                        <a href="https://www.youtube.com/channel/UCC_ds4x9Iv3tcvKi-JdQ-Qw/videos"><h5>Youtube-Guides</h5></a>
                    </div>
                    <div class="guideDescription">
                        Grundlagen des Spiels auf Youtube. Lass dir vom Dev selbst das Spiel erklären.
                    </div>
                    <div class="guideImage">
                        <img src="<?php echo base_url('assets/images/YT-Btn_red.png') ?>" alt="Bild/Symbol" width="50%" >
                    </div>
                </div>
            </div>
        </div>

</div>